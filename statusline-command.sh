...
# Git branch and dirty state — skip optional locks to avoid contention
git_info=""
if git_branch=$(git -C "$cwd" --no-optional-locks symbolic-ref --short HEAD 2>/dev/null); then
  if git -C "$cwd" --no-optional-locks diff --quiet 2>/dev/null && \
    ...
    git_info=" \033[1;34mgit:(\033[0;31m${git_branch}\033[1;34m)\033[1;34m) \033[0;33m✗\033[0m"
  fi
fi
...
# robbyrussell: ➜  dirname git:(branch) ✗
printf "\033[1;32m➜\033[0m  \033[0;36m%s\033[0m%s%s%s" "$dir_name" "$git_info" "$ctx_info" "$model_info"
