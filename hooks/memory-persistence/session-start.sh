...
LATEST=$(find "$HANDOFFS_DIR" -name "*-${PROJECT}-session.tmp" -mtime -7 2>/dev/null | sort -r | head -1)
[ -z "$LATEST" ] && LATEST=$(find "$HANDOFFS_DIR" -name "*.tmp" -mtime -7 2>/dev/null | sort -r | head -1)
