...
7. **Draft action items** — concrete, assignable, ordered. Each action item names the responsible agent/role. Vague actions ("look into X") are not permitted.
   ...
   - `milestone`: the milestone from `product-direction.ctx.md` this action advances. Use the exact name as it appears in that file (e.g., `"auth-revamp"`, `"re-engagement-v1"`). Use `"none"` only if the action is standalone and advances no named milestone — this should be rare; most actions trace to a milestone. Do not invent milestone names — they must exist in `product-direction.ctx.md`.
  ...
  "action_items": [
    {
      ...
      "milestone": "milestone-name-from-product-direction | none",
      "feature_title": "short feature name — used as FEATURE-NNN title in cleared-queue.md",
      "feature_type": "product | engineering | research | cross-functional",
      "feature_acceptance": "what done looks like at feature level — CPO-readable, non-technical, one sentence",
      "feature_validation": "cpo | cpo + cto | cpo + cro | cpo + cro + cto",
