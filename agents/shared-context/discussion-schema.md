...
## CTO Agent Extensions

The CTO has an optional `frame_challenge` field not present in the base schema. It is activated when PF1/PF2 reasoning concludes the Router's framing is constraining. See `~/.claude/agents/cto/persona.md` for the full definition and activation rule. When present, it must appear at the `position` level — not buried in `reasoning[]`.
